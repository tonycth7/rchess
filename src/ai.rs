...
fn pst(k:Kind,r:usize,c:usize)->i32{match k{Kind::P=>PST_P[r][c],Kind::N=>PST_N[r][c],Kind::B=>PST_B[r][c],Kind::R=>PST_R[r][c],Kind::Q=>PST_Q[r][c],Kind::K=>PST_K[r][c]}}

pub fn evaluate(b:&Board)->i32{
    let mut s=0i32;
    for r in 0..8{for c in 0..8{if let Some(p)=b[r][c]{
...
pub fn minimax(b:&Board,depth:u8,mut alpha:i32,mut beta:i32,is_max:bool,ep:Option<(usize,usize)>,cast:&Castle)->i32{
    let color=if is_max{Color::White}else{Color::Black};
    let moves=legal(b,color,ep,cast);
    if depth==0||moves.is_empty(){
        if moves.is_empty(){return if in_check(b,color){if is_max{-99_000}else{99_000}}else{0};}
        return evaluate(b);
    }
    if is_max{
        let mut best=i32::MIN;
        for mv in&moves{let(nb,ne,nc)=apply(b,mv,ep,cast);let v=minimax(&nb,depth-1,alpha,beta,false,ne,&nc);if v>best{best=v;}if best>alpha{alpha=best;}if beta<=alpha{break;}}
        best
    }else{
        let mut best=i32::MAX;
        for mv in&moves{let(nb,ne,nc)=apply(b,mv,ep,cast);let v=minimax(&nb,depth-1,alpha,beta,true,ne,&nc);if v<best{best=v;}if best<beta{beta=best;}if beta<=alpha{break;}}
        best
    }
}

pub fn best_mv(b:&Board,color:Color,ep:Option<(usize,usize)>,cast:&Castle,depth:u8)->Option<Mv>{
    let moves=legal(b,color,ep,cast);if moves.is_empty(){return None;}
    let is_max=color==Color::White;
    let mut bval=if is_max{i32::MIN}else{i32::MAX};let mut best=None;
    for mv in&moves{

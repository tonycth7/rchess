...
use crate::app::{App, Gs, Mode, Screen, ClockState, GameEnd, ReplaySnap, VERSION};
...
fn draw_settings(app: &App, f: &mut Frame) {
    ...
    let (hint,hcol) = if app.saved_notice.is_some() {
        ("  ✓ Saved to ~/.rchess_tui.conf", TEAL)
    } else { ("  w save    r reset    ←→ change value    Esc back", DIMMER) };
...
fn draw_topbar(app: &App, f: &mut Frame, area: Rect) {
    ...
    let book_ind = if app.gs.history.len() < 30 && app.mode == Mode::CPU { " [book]" } else { "" };
    f.render_widget(
        Paragraph::new(format!(" ♟ RCHESS v{}{}{}│  {}  │  {}", VERSION, clock, book_ind, mode_s, st))
...
fn draw_input(app: &App, f: &mut Frame, area: Rect) {
    ...
    let (hint_line,hint_col) = if let Some(err)=&app.input_err {
        ...
        ("  arrows/hjkl cursor  ·  letter starts notation  ·  d draw offer  ·  r replay".to_string(), DIMMER)
    };
...
fn draw_sidebar(app: &App, f: &mut Frame, area: Rect) {
    let [pl,st,hi,kb] = Layout::vertical([
        ...
        Constraint::Min(5),    Constraint::Length(9),
    ]).areas(area);
...
fn draw_players(app: &App, f: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        Line::from(vec![
            ...
            Span::styled(if has_clk{format!("{:>8}",btime.trim())}else{String::new()},
        ...
        Line::from(vec![
            ...
            Span::styled(if has_clk{format!("{:>8}",wtime.trim())}else{String::new()},
...
fn draw_status(app: &App, f: &mut Frame, area: Rect) {
    ...
    let undo_info = if !app.undo_stack.is_empty() {
        format!("  undo: {} avail", app.undo_stack.len())
    } else { String::new() };
    let pgn_info = if let Some(path)=&app.pgn_saved_path {
        format!("  PGN: {}", path.split('/').last().unwrap_or(path))
    } else { String::new() };
    let lines = vec![
        ...
        Line::from(vec![Span::styled(pgn_info, sty(TEAL))]),
    ];
    ...
    if pairs.is_empty() {
        ...
        for (n,w,b) in &pairs[start..] {
            lines.push(Line::from(vec![
                ...
                Span::styled(format!("{:<10}",w), sty(CREAM).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{:<10}",b), sty(DIM)),
            ]));
        }
    }
...
fn draw_keybinds(app: &App, f: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(vec![Span::styled(" click       mouse: select & move", sty(DIM))]),
        Line::from(vec![Span::styled(" any letter  start typing notation", sty(DIM))]),
        Line::from(vec![Span::styled(" d  draw offer (PvP)   u  undo", sty(DIM))]),
        Line::from(vec![Span::styled(" r  replay (after game ends)", sty(DIM))]),
...
fn draw_gameover(app: &App, f: &mut Frame) {
    ...
    let (title,title_col,body) = if flagged {
        ...
        match app.gs.status {
            ...
            Status::Stalemate => (
                ...
                vec!["  The game is drawn.".to_string(), "  No legal moves.".to_string()]),
            _ => return,
        }
    };
    ...
    let pgn_str = if let Some(p)=&app.pgn_saved_path {
        format!("  PGN saved: {}", p.split('/').last().unwrap_or(p))
    } else { String::new() };

    let mut lines = vec![Line::from("")];
    for b in &body { lines.push(Line::from(vec![Span::styled(b.as_str(), sty(CREAM).add_modifier(Modifier::BOLD))])); }
    lines.push(Line::from(""));
    lines.push(Line::from(vec![Span::styled(format!("  {} moves played", total), sty(DIM))]));
    if !pgn_str.is_empty() {
        lines.push(Line::from(vec![Span::styled(&pgn_str, sty(TEAL))]));
    }
    lines.push(Line::from(""));
    lines.push(Line::from(vec![Span::styled(format!("  {:─<40}",""), sty(DIMMER))]));
    lines.push(Line::from(""));
    lines.push(Line::from(vec![
...
fn draw_draw_offer(app: &App, f: &mut Frame) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("  Y / Enter  accept draw", sty(TEAL).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::styled("  N / Esc    decline & keep playing", sty(RED)),
        ]),
    ];
    f.render_widget(Paragraph::new(lines), inner);
}
...
fn draw_replay(app: &App, f: &mut Frame) {
    ...
    if let Some(snap) = snap {
        draw_replay_board(app, f, board_col, snap);
    }
...
fn draw_replay_sidebar(app: &App, f: &mut Frame, area: Rect) {
    let th   = app.cfg.theme;
    let [info_area, hist_area, keys_area] = Layout::vertical([
        Constraint::Length(6), Constraint::Min(5), Constraint::Length(6),
    ]).areas(area);

    // Info
    let snap = app.replay_snaps.get(app.replay_idx);
    let mv_info = if app.replay_idx == 0 {
    ...
    let progress = format!("  Position {}/{}", app.replay_idx, app.replay_snaps.len().saturating_sub(1));
    let info_lines = vec![
    ...
    f.render_widget(
        Paragraph::new(info_lines).block(panel_block("POSITION", th)),
        info_area,
    );

    // History (highlight current move)
    let h   = &app.gs.history;
    let cap = hist_area.height.saturating_sub(2) as usize;
    let mut pairs: Vec<(usize,String,String,bool,bool)> = vec![];
    let mut i=0; let mut n=1;
    while i<h.len() {
        let w_cur = app.replay_idx == i+1;
        let b_cur = app.replay_idx == i+2;
        pairs.push((n, h[i].san.clone(), h.get(i+1).map(|e|e.san.clone()).unwrap_or_default(), w_cur, b_cur));
        i+=2; n+=1;
    }
    let start = pairs.len().saturating_sub(cap);
    let mut hist_lines: Vec<Line> = vec![];
    for (n,w,b,wc,bc) in &pairs[start..] {
        hist_lines.push(Line::from(vec![
            Span::styled(format!(" {:>3}. ",n), sty(DIM)),
            Span::styled(format!("{:<10}",w),
                if *wc{Style::default().fg(bg0(th)).bg(TEAL).add_modifier(Modifier::BOLD)}else{sty(CREAM)}),
            Span::styled(format!("{:<10}",b),
    ...
    // Keys
    let key_lines = vec![
        Line::from(vec![Span::styled(" ←/h  previous move", sty(DIM))]),
        Line::from(vec![Span::styled(" →/l  next move", sty(DIM))]),
        Line::from(vec![Span::styled(" 0    start position", sty(DIM))]),
        Line::from(vec![Span::styled(" $    final position", sty(DIM))]),
        Line::from(vec![Span::styled(" q/Esc  back to game", sty(DIM))]),
    ];
    f.render_widget(Paragraph::new(key_lines).block(panel_block("REPLAY KEYS", th)), keys_area);
}
